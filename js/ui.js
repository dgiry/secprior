      ...
      <article class="card crit-${article.criticality} priority-${article.priorityLevel || 'low'}${article._isNew ? " card-new" : ""}${isRead ? " card-read" : ""}${isReviewed ? " card-reviewed" : ""}" data-id="${article.id}"
        ...
        <div class="card-pipeline">${(() => {
          ...
          return [
            ...
            `<a href="https://dgiry.github.io/ttps-mapper?context=${encodeURIComponent(ctx.slice(0,300))}" target="_blank" rel="noopener" class="pipeline-link" onclick="event.stopPropagation()" title="Map ATT&CK TTPs">🗺️ TTPs</a>`,
