...
const AlertManager = (() => {
  ...
  const DEFAULTS = {
    ...
    // Resend
    resendApiKey:    "",
    resendFrom:      "",         // ex: "SecPrior <alerts@votredomaine.fr>"
    // SendGrid
    sendgridApiKey:  "",
  ...
  function formatAlertHTML(articles) {
    ...
    return `
      <div style="font-family:monospace;background:#0d1117;color:#e6edf3;padding:20px;border-radius:8px">
        <h2 style="color:#f85149;margin-top:0">🛡️ SecPrior — Security Alert</h2>
        ...
        <p style="color:#8b949e;font-size:12px;margin-top:16px">
          Sent by SecPrior · ${new Date().toLocaleString("en-US")}
        </p>
      </div>`;
  }
    ...
    return `
      <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;
        ...
        <div style="border-bottom:1px solid #30363d;padding-bottom:16px;margin-bottom:20px">
          ...
          <p style="margin:0;color:#8b949e;font-size:13px">SecPrior · Digest ${label}</p>
        </div>
        <!-- Executive summary -->
        ...
        <div style="border-top:1px solid #30363d;margin-top:24px;padding-top:16px;text-align:center">
          <p style="margin:0;color:#8b949e;font-size:11px">
            SecPrior · ${new Date().toLocaleString("en-US")} ·
  ...
  function _formatBriefingText(top, rest, label, stats = null) {
    ...
    t += `${sep60}\nGenerated by SecPrior on ${new Date().toLocaleString("en-US")}\n`;
    return t;
  }
  ...
  async function sendWebhook(articles, settings, subjectOverride) {
    ...
    const payload = {
      ...
      text:       `🛡️ *SecPrior* — ${articles.length} new alert(s)` +
  ...
  async function sendEmailJS(articles, settings, subjectOverride, htmlOverride, textOverride) {
    ...
    await window.emailjs.send(emailjsService, emailjsTemplate, {
      ...
      subject:       subjectOverride || `🛡️ SecPrior — ${articles.length} high-priority alert(s)`,
  ...
  const RESEND_SANDBOX_FROM = "SecPrior <onboarding@resend.dev>";
  ...
  async function sendResend(articles, settings, subjectOverride, htmlOverride, textOverride) {
    ...
    const subject = subjectOverride || `🛡️ SecPrior — ${articles.length} high-severity alert(s)`;
    ...
    if (CONFIG.USE_API) {
      ...
      const res = await fetch("https://api.sendgrid.com/v3/mail/send", {
        ...
        body:    JSON.stringify({
          ...
          from:    { email: sendgridFrom, name: "SecPrior" },
  ...
  function sendMailto(articles, settings, subjectOverride, textOverride) {
    const subject = encodeURIComponent(
      subjectOverride || `🛡️ SecPrior — ${articles.length} cybersecurity alert(s)`
    );
    const body = encodeURIComponent(
      textOverride || (
        `SecPrior — High severity alerts\n` +
